...
AnalysisCompositeCut* o2::aod::dqcuts::GetCompositeCut(const char* cutName)
{
  ...
  if (!nameStr.compare("Jpsi_TPCPost_calib_debug6_loose")) {
    cut->AddCut(GetAnalysisCut("jpsi_trackCut_debug2"));
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6_loose"));
    return cut;
  }

  if (!nameStr.compare("Jpsi_TPCPost_calib_debug6_pid1")) {
    cut->AddCut(GetAnalysisCut("jpsi_trackCut_debug2"));
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6_pid1"));
    return cut;
  }

  if (!nameStr.compare("Jpsi_TPCPost_calib_debug6_pid2")) {
    cut->AddCut(GetAnalysisCut("jpsi_trackCut_debug2"));
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6_pid2"));
    return cut;
  }

  if (!nameStr.compare("Jpsi_TPCPost_calib_debug6_pid3")) {
    cut->AddCut(GetAnalysisCut("jpsi_trackCut_debug2"));
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6_pid3"));
    return cut;
  }
  ...
  //////// Cuts for Upsilon ////////
  if (!nameStr.compare("UpsilonCut1")) {
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6"));
    cut->AddCut(GetAnalysisCut("jpsi_trackCut_debug2"));
    cut->AddCut(GetAnalysisCut("UpsilonKineCuts"));
    return cut;
  }

  if (!nameStr.compare("UpsilonCut1_withoutPionRejection")) {
    cut->AddCut(GetAnalysisCut("jpsi_trackCut_debug2"));
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6_withoutPionRejection"));
    cut->AddCut(GetAnalysisCut("UpsilonKineCuts"));
    return cut;
  }

  if (!nameStr.compare("UpsilonMassCut1")) {
    cut->AddCut(GetAnalysisCut("pairUpsilonWide"));
    return cut;
  }

  if (!nameStr.compare("jpsiO2MCdebugCuts2_withoutPionRejection")) {
    cut->AddCut(GetAnalysisCut("jpsiStandardKine"));
    cut->AddCut(GetAnalysisCut("electronStandardQualityForO2MCdebug"));
    cut->AddCut(GetAnalysisCut("jpsi_TPCPID_debug6_withoutPionRejection"));
    return cut;
  ...
  //////////////////// Cuts for Pc4312 ////////////////////
  if (!nameStr.compare("protonPc")) {
    cut->AddCut(GetAnalysisCut("protonPIDNsigma"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    return cut;
  }

  if (!nameStr.compare("protonPcPos")) {
    cut->AddCut(GetAnalysisCut("protonPIDNsigma"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("posTrack"));
    return cut;
  ...
  if (!nameStr.compare("protonPcWithTOF")) {
    cut->AddCut(GetAnalysisCut("protonPID_TPCnTOF"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    return cut;
  }
  ...
  if (!nameStr.compare("protonPcPosWithTOF")) {
    cut->AddCut(GetAnalysisCut("protonPID_TPCnTOF"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("posTrack"));
    return cut;
  ...
  if (!nameStr.compare("pionPc")) {
    cut->AddCut(GetAnalysisCut("pionPIDnsigma"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    return cut;
  }

  if (!nameStr.compare("pionPcWithTOF")) {
    cut->AddCut(GetAnalysisCut("pionPID_TPCnTOF"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    return cut;
  }

  if (!nameStr.compare("pionPcPos")) {
    cut->AddCut(GetAnalysisCut("pionPIDnsigma"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("posTrack"));
    return cut;
  ...
  if (!nameStr.compare("pionPcPosWithTOF")) {
    cut->AddCut(GetAnalysisCut("pionPID_TPCnTOF"));
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("posTrack"));
    return cut;
  ...
  if (!nameStr.compare("flowTrackQualityQuarterEta1")) {
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("pTCutFlow"));
    cut->AddCut(GetAnalysisCut("quarterEta1"));
    return cut;
  }

  if (!nameStr.compare("flowTrackQualityQuarterEta2")) {
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("pTCutFlow"));
    cut->AddCut(GetAnalysisCut("quarterEta2"));
    return cut;
  }

  if (!nameStr.compare("flowTrackQualityQuarterEta3")) {
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("pTCutFlow"));
    cut->AddCut(GetAnalysisCut("quarterEta3"));
    return cut;
  }

  if (!nameStr.compare("flowTrackQualityQuarterEta4")) {
    cut->AddCut(GetAnalysisCut("QuarkaniumTrackQuality"));
    cut->AddCut(GetAnalysisCut("pTCutFlow"));
...
AnalysisCut* o2::aod::dqcuts::GetAnalysisCut(const char* cutName)
{
  ...
  if (!nameStr.compare("jpsi_TPCPID_debug6_pid1")) {
    cut->AddCut(VarManager::kTPCnSigmaEl, -3.0, 3.0);
    cut->AddCut(VarManager::kTPCnSigmaPi, 2.7, 999);
    cut->AddCut(VarManager::kTPCnSigmaPr, 2.7, 999);
    return cut;
  }

  if (!nameStr.compare("jpsi_TPCPID_debug6_pid2")) {
    cut->AddCut(VarManager::kTPCnSigmaEl, -3.0, 3.0);
    cut->AddCut(VarManager::kTPCnSigmaPi, 2.5, 999);
    cut->AddCut(VarManager::kTPCnSigmaPr, 2.5, 999);
    return cut;
  ...
  if (!nameStr.compare("jpsi_TPCPID_debug6_pid3")) {
    cut->AddCut(VarManager::kTPCnSigmaEl, -2.0, 3.0);
  ...
  if (!nameStr.compare("jpsi_TPCPID_debug6_withoutPionRejection")) {
    cut->AddCut(VarManager::kTPCnSigmaEl, -2.0, 3.0);
    cut->AddCut(VarManager::kTPCnSigmaPr, 3.0, 999);
    return cut;
  }
  ...
  if (!nameStr.compare("pairUpsilonWide")) {
    cut->AddCut(VarManager::kMass, 5.0, 14.0);
    return cut;
  }
  ...
  ///////////////////// Cuts for Upsilon ///////////////////////
  if (!nameStr.compare("QuarkaniumTrackQuality")) {
    cut->AddCut(VarManager::kTPCchi2, 0.0, 4.0);
    cut->AddCut(VarManager::kTPCncls, 90., 159);
    cut->AddCut(VarManager::kITSncls, 2.5, 7.5);
    cut->AddCut(VarManager::kIsSPDany, 0.5, 1.5);
  ...
  if (!nameStr.compare("UpsilonKineCuts")) {
    cut->AddCut(VarManager::kPt, 3.0, 1000.0);
    return cut;
  }

  ////////////////////// Cuts for Pc4312 ///////////////////////
  if (!nameStr.compare("protonPIDNsigma")) {
    cut->AddCut(VarManager::kTPCnSigmaPr, -3.0, 3.0);
    return cut;
  }
  ...
  if (!nameStr.compare("quarterEta2")) {
    cut->AddCut(VarManager::kEta, -0.45, 0.0);
    return cut;
  }

  if (!nameStr.compare("quarterEta3")) {
    cut->AddCut(VarManager::kEta, 0.0, 0.45);
    return cut;
  }
  ...
  if (!nameStr.compare("pTCutFlow24")) {
    cut->AddCut(VarManager::kPt, 0.2, 6.0);
    return cut;
  }

  if (!nameStr.compare("primaryTrackDCAcutsPtDependent")) {
    std::shared_ptr<TF1> fHighDCAxyDepPt = std::make_shared<TF1>("f_highDCAxy_depPt", [](double* x, double* p) { return p[0] / std::pow(x[0] - p[1], p[2]); }, 0.1, 4, 3);
    std::shared_ptr<TF1> fLowDCAxyDepPt = std::make_shared<TF1>("f_lowDCAxy_depPt", [](double* x, double* p) { return p[0] / std::pow(x[0] - p[1], p[2]); }, 0.1, 4, 3);
    std::shared_ptr<TF1> fHighDCAzDepPt = std::make_shared<TF1>("f_highDCAz_depPt", [](double* x, double* p) { return p[0] / std::pow(x[0] - p[1], p[2]); }, 0.1, 4, 3);
    std::shared_ptr<TF1> fLowDCAzDepPt = std::make_shared<TF1>("f_lowDCAz_depPt", [](double* x, double* p) { return p[0] / std::pow(x[0] - p[1], p[2]); }, 0.1, 4, 3);

    double pXY[3] = {0.00439545, 0.095, 0.666164};
    double pXYMinus[3] = {-0.00439545, 0.095, 0.666164};
    double pZ[3] = {0.00495253, 0.081471, 0.705002};
    double pZMinus[3] = {-0.00495253, 0.081471, 0.705002};
    fHighDCAxyDepPt->SetParameters(pXY);
    fLowDCAxyDepPt->SetParameters(pXYMinus);
    fHighDCAzDepPt->SetParameters(pZ);
    fLowDCAzDepPt->SetParameters(pZMinus);

    cut->AddCut(VarManager::kTrackDCAxy, fLowDCAxyDepPt, fHighDCAxyDepPt, false, VarManager::kPt, 0.1, 4.0, false);
    cut->AddCut(VarManager::kTrackDCAz, fLowDCAzDepPt, fHighDCAzDepPt, false, VarManager::kPt, 0.1, 4.0, false);
    return cut;
  }

  if (!nameStr.compare("primaryTrackDCAcutsPtDependent24")) {
    std::shared_ptr<TF1> fHighDCADepPt = std::make_shared<TF1>("f_highDCA_depPt", "4.*(0.00179344+ 0.000924651 * pow(1./x, 1.4062))", 0.1, 6);
    std::shared_ptr<TF1> fLowDCADepPt = std::make_shared<TF1>("f_lowDCA_depPt", "-4.*(0.00179344+ 0.000924651 * pow(1./x, 1.4062))", 0.1, 6);

    cut->AddCut(VarManager::kTrackDCAxy, fLowDCADepPt, fHighDCADepPt, false, VarManager::kPt, 0.1, 6.0, false);
    cut->AddCut(VarManager::kTrackDCAz, fLowDCADepPt, fHighDCADepPt, false, VarManager::kPt, 0.1, 6.0, false);
    return cut;
  }

  if (!nameStr.compare("primaryTrackDCAcutsPtDependentLoose")) {
    std::shared_ptr<TF1> fHighDCADepPt = std::make_shared<TF1>("f_highDCA_depPt", "6.*(0.00179344+ 0.000924651 * pow(1./x, 1.4062))", 0.1, 6);
    std::shared_ptr<TF1> fLowDCADepPt = std::make_shared<TF1>("f_lowDCA_depPt", "-6.*(0.00179344+ 0.000924651 * pow(1./x, 1.4062))", 0.1, 6);
